...
class NavBar:
    ...
    def __init__(self, root, controller, width, height):
        ...
        self.navbar_frame.grid(row=0, column=0, sticky=tk.W, padx=NavBar.PAD_X)
        self.home_btn = tk.Button(self.navbar_frame, text="Home")
        self.home_btn.pack(side=tk.LEFT, padx=NavBar.PAD_X, pady=NavBar.PAD_Y)
        self.back_btn = tk.Button(self.navbar_frame, text="Back")
        self.back_btn.pack(side=tk.LEFT, padx=NavBar.PAD_X, pady=NavBar.PAD_Y)
        self.forward_btn = tk.Button(self.navbar_frame, text="Forward")
        self.forward_btn.pack(side=tk.LEFT, padx=NavBar.PAD_X,
                              pady=NavBar.PAD_Y)

        self.path_field = tk.Entry(self.navbar_frame, width=int(0.124 * width))
        self.path = tk.StringVar()
        self.path_field.configure(textvariable=self.path)
        self.path_field.pack(side=tk.RIGHT, ipady=NavBar.IPADDING,
                             pady=NavBar.PAD_Y)
...
class Viewer:
    ...
    def __init__(self, root, controller, width, height):
        ...
        self.folders = []
        self.files = []
        # self.tempFolder = Folder(self.viewer_frame, controller, "Lior", 0, 0)
        # self.tempFile = File(self.viewer_frame, controller, "Noam", self.width, self.height, 0, 1)
...
class Folder:
    ...
    def __init__(self, root, folder_name, row, col):
        ...
        self.folder_frame.grid(row=row, column=col, padx=Viewer.PADDING * 2,
                               pady=Viewer.PADDING * 2)
...
class File:
    ...
    def __init__(self, root, file_name, row, col):
        ...
        self.file_frame.grid(row=row, column=col, padx=Viewer.PADDING * 2,
                             pady=Viewer.PADDING * 2)
